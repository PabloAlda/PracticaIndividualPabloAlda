...
SPEEDUP = 5.0                # x5 velocidad simulada
...
# Límite de líneas CSV (no se usa aquí) y límite de vehículos en escena (prevenir saturación)
MAX_VEHICLES = 2000

# -------------------------
# LÍMITES VELOCIDAD POR CARRIL (km/h -> m/s)
# carriles index: 0 = izquierdo (rápido), 1 = medio, 2 = derecho (lento)
...
def run_pygame_sim():
    ...
    while running:
        ...
        # stop condition: sim_time passes SIM_SECONDS
        if sim_time >= SIM_SECONDS:
            running = False

    # End simulation: compute final stats
    ...
    # keep final window until closed
